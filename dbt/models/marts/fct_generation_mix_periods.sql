{{
  config(
    materialized = 'table',
    description  = '''
      Average daily generation (GWh) per energy source, split by before/after
      the April 15 2023 nuclear phase-out. Primary model for Looker Studio
      Tile 1: Before vs After stacked bar chart.
...
with daily_by_source as (
    select
        ...
        comparison_window,
        date,
        -- Daily resolution: SMARD already gives MWh per day, just convert to GWh
        mwh / 1000.0                    as gwh_day
    ...
    select
        *,
        100.0 * avg_gwh_per_day
            / nullif(
                sum(case when category like 'generation%' then avg_gwh_per_day else 0 end)
                over (partition by comparison_window),
              0)                        as pct_of_generation
