...
with daily_prices as (
    select
        ...
        -- Count negative price days (interesting signal post phase-out)
        countif(eur_per_mwh < 0)    as negative_price_days,
        count(*)                    as total_price_days
    from {{ ref('fct_daily_prices_base') }}
    group by 1, 2, 3, 4, 5
),
...
pivoted as (
    select
        ...
        max(case when country = 'Germany' then negative_price_days end)       as de_negative_days,
        max(case when country = 'France'  then negative_price_days end)       as fr_negative_days
    from daily_prices
    group by 1, 2, 3, 4
)
...
select
    ...
    -- Rolling 30-day average to smooth daily noise in Looker Studio
